...
class NuScenesDataset(Custom3DDataset):
    ...
    def _evaluate_single(
        ...
        import nuscenes.eval.detection.evaluate as det_eval_module
        ...
        std_token_count = sum(
            1 for s in nusc.sample
            if nusc.get("scene", s["scene_token"])["name"]
            in nusc_splits.create_splits_scenes()[eval_set]
        )
        is_subset = len(pred_tokens) < std_token_count

        if is_subset:
            _orig_load_gt = nusc_loaders.load_gt
            _pred_tokens = pred_tokens

            def _patched_load_gt(nusc_, eval_split, box_cls, verbose=False):
                full_gt = _orig_load_gt(nusc_, eval_set, box_cls, verbose)
                filtered = EvalBoxes()
                for token in _pred_tokens:
                    filtered.add_boxes(token, full_gt[token] if token in full_gt.boxes else [])
                return filtered

            nusc_loaders.load_gt = _patched_load_gt
            det_eval_module.load_gt = _patched_load_gt
        ...
        if is_subset:
            nusc_loaders.load_gt = _orig_load_gt
            det_eval_module.load_gt = _orig_load_gt
