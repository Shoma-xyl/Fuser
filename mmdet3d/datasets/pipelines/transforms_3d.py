...
class LidarOcclusion:
    """Apply deterministic LiDAR occlusion to loaded point clouds at test time."""
    ...
    def __init__(
        ...
        assert mode in ["none", "pointdrop", "spatial", "angular"]
        assert region in ["front", "back", "left", "right"]
        self.mode = mode
    ...
    def _sample_seed(self, data):
        key = str(data.get("token", data.get("sample_idx", data.get("timestamp", ""))))
        value = int(hashlib.md5(key.encode("utf-8")).hexdigest()[:8], 16)
        return (value + self.seed) % (2**32)

    def _region_mask(self, point_tensor):
        x = point_tensor[:, 0]
        y = point_tensor[:, 1]
        if self.region == "front":
            return x > 0
        if self.region == "back":
            return x < 0
        if self.region == "left":
    ...
    def _angle_mask(self, point_tensor):
        x = point_tensor[:, 0]
        y = point_tensor[:, 1]
        theta = torch.atan2(y, x) * (180.0 / np.pi)
        half_range = self.angle_range / 2.0
        if self.region == "front":
            return (x > 0) & (torch.abs(theta) <= half_range)
        if self.region == "back":
            return (x < 0) & (
                (torch.abs(theta - 180.0) <= half_range)
                | (torch.abs(theta + 180.0) <= half_range)
            )
        if self.region == "left":
            return (y > 0) & (torch.abs(theta - 90.0) <= half_range)
        return (y < 0) & (torch.abs(theta + 90.0) <= half_range)

    def __call__(self, data):
        points = data["points"]
        num_points = points.tensor.shape[0]
        if self.mode == "none" or num_points == 0 or self.drop_percentage <= 0:
            return data

        if self.mode == "pointdrop":
            candidate = torch.ones(num_points, dtype=torch.bool, device=points.tensor.device)
        elif self.mode == "spatial":
            candidate = self._region_mask(points.tensor)
        else:
            candidate = self._angle_mask(points.tensor)

        candidate_idx = torch.nonzero(candidate, as_tuple=False).squeeze(1)
        num_drop = int(candidate_idx.numel() * self.drop_percentage / 100.0)
        if num_drop <= 0:
            return data

        rng = np.random.RandomState(self._sample_seed(data))
        drop_np = rng.choice(candidate_idx.cpu().numpy(), num_drop, replace=False)
        keep = torch.ones(num_points, dtype=torch.bool, device=points.tensor.device)
        keep[torch.as_tensor(drop_np, dtype=torch.long, device=points.tensor.device)] = False
    ...
    def __repr__(self):
        return (
            ...
            f"mode={self.mode}, drop_percentage={self.drop_percentage}, "
            f"region={self.region}, angle_range={self.angle_range}, seed={self.seed})"
        )
